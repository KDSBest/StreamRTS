...
namespace ClipperLib
{
  ...
  public class ClipperOffset
  {
    ...
    private List<IntPoint> m_normals = new List<IntPoint>();
    ...
    internal static IntPoint GetUnitNormal(IntPoint pt1, IntPoint pt2)
    {
      int dx = (pt2.X - pt1.X);
        int dy = (pt2.Y - pt1.Y);
      if ((dx == 0) && (dy == 0)) return new IntPoint();
    ...
    private void DoOffset(double delta)
    {
      ...
      for (int i = 0; i < m_polyNodes.ChildCount; i++)
      {
        ...
        if (node.m_endtype == EndType.etClosedLine || 
          ...
          m_normals.Add(new IntPoint(m_normals[len - 2]));
        ...
        else if (node.m_endtype == EndType.etClosedLine)
        {
          ...
          m_destPoly = new Path();
                    //re-build m_normals ...
            IntPoint n = m_normals[len - 1];
          for (int j = len - 1; j > 0; j--)
            m_normals[j] = new IntPoint(-m_normals[j - 1].X, -m_normals[j - 1].Y);
          m_normals[0] = new IntPoint(-n.X, -n.Y);
          k = 0;
          for (int j = len - 1; j >= 0; j--)
            OffsetPoint(j, ref k, node.m_jointype);
          m_destPolys.Add(m_destPoly);
        ...
        else
        {
          ...
          else
          {
            ...
            k = len - 2;
            m_sinA = 0;
            m_normals[j] = new IntPoint(-m_normals[j].X, -m_normals[j].Y);
          ...
          m_normals[0] = new IntPoint(-m_normals[1].X, -m_normals[1].Y);
